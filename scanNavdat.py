...
def normArgs(argv):
  ...
  global cifsAll, fldrTree, ndatPFId, outpPath, specPFid, verbose, wantHelp
# fallback values
  ndatPFId  = '/data/navNumb.dat'
  outpPath  = '/data/Airports'
  ...
  # get args
  try:
    opts, args = getopt.getopt(argv, "hn:o:s:v", \
      [ "help", "ndatPFId",  "outpPath=" , "specFile=", "--verbose" ] )
...
def get_magnVari( tIcao) :
  global Icao, magnVari, outpPath, locsXmlOpen
...
def parseRway ( tRow) :
  global Icao, magnVari, outpPath, nvdbHndl, locsXmlOpen, thrsElvM
...
def parseLocs ( tRow, xRway) :
  global Icao, magnVari, outpPath, locsXmlOpen, thrsElvM
  locsRwy    = tRow[9][2:]
  locsNvId   = tRow[5]
...
def mill_rwys(tIcao):
  ...
  global magnVari, outpPath, locsXmlOpen
  global xThlds, xRway, xThrs, xIden, xHdng, xLati, xLong, xDisp, xStop
  rwysDone = []
  locsPropOpen = 0
  locsRwayOpen = 0
  ...
  if ( not (rwayIcao == '' )) :
    ...
    if ( fldrTree > 0 ) :
      if (( len(rwayIcao) == 4)) :
        fldrPath   = ("%s/%s/%s/%s" % (outpPath, rwayIcao[0], rwayIcao[1], rwayIcao[2] ))
      if (( len(rwayIcao) == 3)) :
        fldrPath   = ("%s/%s/%s" % (outpPath, rwayIcao[0], rwayIcao[1] ))
      if (( len(rwayIcao) == 2)) :
        fldrPath   = ("%s/%s" % (outpPath, rwayIcao[0]))
      #
      if ( not ( os.path.isdir( fldrPath ))) :
         os.makedirs( fldrPath )
      thldXmlFid = ("%s/%s.threshold.xml" % (fldrPath, rwayIcao))
    else:
      thldXmlFid = ("%s/%s.threshold.xml" % (outpPath, rwayIcao))
    #print(thrsXmlFid)
    ...
    if ( locsPropOpen > 0 ) :
      if ( fldrTree > 0 ) :
        if (( len(rwayIcao) == 4)) :
          locsXmlFid = ("%s/%s/%s/%s/%s.ils.xml" % (outpPath, rwayIcao[0], rwayIcao[1], rwayIcao[2], rwayIcao))
        else :
          locsXmlFid = ("%s/%s/%s/%s.ils.xml" % (outpPath, rwayIcao[0], rwayIcao[1],                 rwayIcao))
      else:
        locsXmlFid = ("%s/%s.ils.xml" % (outpPath, rwayIcao))
...
if __name__ == '__main__':
  ...
  if (wantHelp > 0 ) :
    ...
    nAddPFId = ( outpPath + '/../NavData/nav/addins-nav.dat' )
    #naddHndl = open( naddPFId, 'a' )
    #
    with open(ndatPFId) as ndatFile:
      #ndatFile.seek(3)
      for ndatLine in ndatFile:
        ndatList = list( ndatLine.split() ) 
        if ((len(ndatLine) > 1 ) and ( int (ndatList[0])  < 10)) :
          ndatLati = float(ndatList[1])
          # approx extents of US data
          if (ndatLati > 20 ) :
            ndatLong = float(ndatList[2])
            #print( ndatList[2], '  ', ndatLong)
            # AWK Wake Isl: Long +166 but try all west of Newfndlnd
            if (ndatLong < -60 ) :
              ndatCode = str(ndatList[0])
              ndatFMsl = ndatList[3]
              ndatFreq = ndatList[4]
              ndatRngm = ndatList[5]
              if ( ndatCode == '2' ) :
              ...
              if ( ndatCode == '3' ) :
                ...
                with dbConn.cursor() as cur:
                  tQuery = "SELECT * FROM cycle2403.vhf_navaid \
                   WHERE  vor_identifier='%s'" % ndatIden
                  cur.execute(tQuery)
                  if (cur.rowcount != 1) :
                    print("rowcount: ", cur.rowcount, '  nav.dat: ', ndatLine)
                  else :  
                    cifsRow = cur.fetchone()
                    #print (' nav.dat: ', ndatList)
                    #print ('cifs.dat: ', cifsRow)

              if (( ndatCode == '4' ) or ( ndatCode == '5' )) :
                # 4:LOC 5:No GS LOC, LDA, SDF
                ndatBngT = ndatList[6]
                ndatIden = ndatList[7]
                ndatAirp = ndatList[8]
                ndatRway = ndatList[9]
                ndatName = ndatList[10:]
              if ( ndatCode == '3' ) :
                #GS
                ndatBngT = ndatList[6][0:3]
                ndatFreq = ndatList[6][3:]
                ndatIden = ndatList[7]
                ndatAirp = ndatList[8]
                ndatRway = ndatList[9]
                ndatName = ndatList[10:]
                ndatName = ndatList[8:]
              if (( ndatCode == '7' ) or ( ndatCode == '8') or ( ndatCode == '9' )) :
                #Mrkr Beacon
                ndatBngT = ndatList[6]
                ndatIden = ndatList[7]
                ndatAirp = ndatList[8]
